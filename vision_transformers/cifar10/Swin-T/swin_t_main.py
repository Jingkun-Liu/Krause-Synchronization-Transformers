...
from module import WarmupScheduler, SoftTargetCrossEntropy, get_mixup_cutmix_transforms
...
def visualize_comprehensive_analysis(swin_results, save_path='swin_analysis.png'):
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))
    
    ax1.plot(swin_results['train_accs'], 'o-', label='SwinTransformer Train', linewidth=2)
    ax1.plot(swin_results['test_accs'], 's-', label='SwinTransformer Test', linewidth=2)
    ...
    ax2.plot(swin_results['train_losses'], 'o-', label='SwinTransformer', linewidth=2)
    ...
    plt.suptitle('SwinTransformer Training Results',
...
def run_swin_evaluation(args):
    print("\n" + "="*70)
    print("SWIN TRANSFORMER-TINY TRAINING BENCHMARK (CIFAR-10)")
    print("="*70 + "\n")
    ...
    swin_model = SwinTransformer(
        ...
        num_heads=[3, 6, 12, 24],
        window_size=[4, 4],
        stochastic_depth_prob=args.drop_path,
        num_classes=10
    )
    ...
    try:
        ...
        with warnings.catch_warnings():
            ...
            flop_counter = FlopCountAnalysis(swin_model, (dummy_input,))
            flops_swin = flop_counter.total()
        print(f"  FLOPs: {int(flops_swin):,} ({flops_swin/1e9:.2f} GFLOPs)")
    ...
    print("\n" + "="*70)
    print("Training SwinTransformer-tiny" + (" (with warmup)" if args.warmup_epochs > 0 else ""))
    print("="*70)

    swin_optimizer = torch.optim.AdamW(swin_model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    swin_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(swin_optimizer, T_max=args.epochs-args.warmup_epochs)
    swin_warmup_scheduler = WarmupScheduler(swin_optimizer, warmup_steps=warmup_steps, start_lr=0.0, target_lr=args.lr) if args.warmup_epochs > 0 else None
    ...
    for epoch in range(args.epochs):
        current_warmup_scheduler = swin_warmup_scheduler if (swin_warmup_scheduler and swin_warmup_scheduler.is_warming_up()) else None
        train_loss, train_acc = train_epoch(
            swin_model, train_loader, swin_optimizer, criterion, device,
        ...
        test_loss, test_acc = evaluate(swin_model, test_loader, criterion, device,
                                       num_classes=10, 
                                       label_smoothing=args.label_smoothing)
        
        swin_results['train_losses'].append(train_loss)
        swin_results['train_accs'].append(train_acc)
        swin_results['test_losses'].append(test_loss)
        swin_results['test_accs'].append(test_acc)

        if swin_warmup_scheduler is None or not swin_warmup_scheduler.is_warming_up():
            swin_scheduler.step()

        if test_acc > best_swin_acc:
            best_swin_acc = test_acc
            best_swin_epoch = epoch + 1
            torch.save(swin_model.state_dict(), 'best_swin_cifar10.pth')
            print(f"  → Saved best SwinTransformer weights (Epoch {epoch+1}, Test Acc: {test_acc:.2f}%)")
        ...
        if (epoch + 1) % 1 == 0:
            ...
            print(f"Epoch {epoch+1}/{args.epochs} - Train: {train_acc:.2f}% - Test: {test_acc:.2f}% - Best: {best_swin_acc:.2f}%{lr_info}")

    print("\n" + "="*70)
    print("LOADING BEST SWIN MODEL FOR FINAL EVALUATION")
    print("="*70)

    print("\n[Loading best SwinTransformer weights]")
    state_dict = torch.load('best_swin_cifar10.pth', map_location=device)
    swin_model.load_state_dict(state_dict)
    swin_model.eval()
    best_swin_loss, best_swin_acc = evaluate(swin_model, test_loader, criterion, device,
                                              num_classes=10,
                                              label_smoothing=args.label_smoothing)
    print(f"SwinTransformer Best Model (Epoch {best_swin_epoch}):")
    print(f"  Test Loss: {best_swin_loss:.4f}, Test Acc: {best_swin_acc:.2f}%")
    ...
    print(f"  SwinTransformer-tiny: Best Test Accuracy = {best_swin_acc:.2f}% (Epoch {best_swin_epoch})")
    
    print("\n" + "="*70)
    print("Generating Visualizations...")
    print("="*70)
    
    visualize_comprehensive_analysis(swin_results, args.save_path)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train SwinTransformer-tiny on CIFAR-10')
    ...
    print(f"Model: SwinTransformer-tiny, img_size={args.img_size}")
