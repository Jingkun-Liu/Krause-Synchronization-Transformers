...
class KrauseViTAttention(nn.Module):
    def __init__(self, d_model, n_heads, sigma=1.0, top_k=5, dropout=0.1, grid_size=8):
        super().__init__()
    ...
    def forward(self, x):
        ...
        consensus = torch.matmul(attn_weights, V)
...
class KrauseViTBlock(nn.Module):
    def __init__(self, d_model, n_heads, d_ff, sigma=1.0, top_k=5, 
                 dropout=0.1, grid_size=8, drop_path=0.0):
        super().__init__()
...
class KrauseVisionTransformer(nn.Module):
    def __init__(self, img_size=32, patch_size=4, in_channels=3, num_classes=10,
                 ...
                 sigma=1.0, top_k=10, dropout=0.1, drop_path=0.0):
        super().__init__()
        ...
        print(f"Krause-ViT: Initializing with {n_layers} layers and varying top_k from {top_k_min_val} to {top_k_max_val}: {top_k_values}")
