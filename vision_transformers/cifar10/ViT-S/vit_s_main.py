...
def visualize_training_curves(results, save_path='vit_training_curves.png'):
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 5))
    
    ax1.plot(results['train_accs'], 'o-', label='Train', linewidth=2)
    ax1.plot(results['test_accs'], 's-', label='Test', linewidth=2)
    ...
    ax2.plot(results['train_losses'], 'o-', label='Train Loss', linewidth=2)
    ax2.plot(results['test_losses'], 's-', label='Test Loss', linewidth=2)
    ...
    plt.suptitle('ViT-S Training Curves',
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train ViT-S on CIFAR-10')
    ...
    parser.add_argument('--save_path', type=str, default='vit_training_curves.png',
                       help='Path to save visualization (default: vit_training_curves.png)')
    ...
    def run_vit_evaluation(
        ...
        print("\n" + "="*70)
        print("VISION TRANSFORMER (ViT-S) EVALUATION")
        print("="*70 + "\n")
        ...
        print("\n" + "="*70)
        print("Training ViT-S" + (" (with warmup)" if warmup_epochs > 0 else ""))
        print("="*70)
        ...
        for epoch in range(n_epochs):
            ...
            if test_acc > best_acc:
                best_acc = test_acc
                best_epoch = epoch + 1
                torch.save(model.state_dict(), 'best_vit_cifar10_vits16.pth')
                print(f"  → Saved best ViT-S weights (Epoch {epoch+1}, Test Acc: {test_acc:.2f}%)")
            
            current_lr = optimizer.param_groups[0]['lr']
            if (epoch + 1) % 1 == 0:
                lr_info = f" - LR: {current_lr:.6f}" if warmup_epochs > 0 and epoch < warmup_epochs else ""
                print(f"Epoch {epoch+1}/{n_epochs} - Train: {train_acc:.2f}% - Test: {test_acc:.2f}% - Best: {best_acc:.2f}%{lr_info}")
        
        print("\n" + "="*70)
        print("LOADING BEST MODEL FOR FINAL EVALUATION")
        print("="*70)

        print("\n[Loading best ViT-S weights]")
        model.load_state_dict(torch.load('best_vit_cifar10_vits16.pth', map_location=device))
        model.eval()
        ...
        print(f"ViT-S Best Model (Epoch {best_epoch}):")
        print(f"  Test Loss: {best_loss:.4f}, Test Acc: {best_acc:.2f}%")
        
        print("\n" + "="*70)
        print("Generating Visualizations...")
        print("="*70)
        
        visualize_training_curves(results, save_path)
    ...
    print(f"Model: img_size={args.img_size}, patch_size={args.patch_size}, d_model={args.d_model}, n_heads={args.n_heads}, n_layers={args.n_layers}")
    print(f"Training: epochs={args.epochs}, lr={args.lr}, weight_decay={args.weight_decay}, warmup_epochs={args.warmup_epochs}")
    print(f"mixup_alpha={args.mixup_alpha}, cutmix_alpha={args.cutmix_alpha}, label_smoothing={args.label_smoothing}")
