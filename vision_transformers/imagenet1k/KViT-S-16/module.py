...
class KrauseViTAttention(nn.Module):
    ...
    def rbf_attention_weights(self, query, key):
        B, H, N, D = query.shape
        sigma_sq = torch.exp(2 * self.log_sigma) # sigma^2
...
class KrauseVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_channels=3, num_classes=1000,
                 d_model=384, n_heads=6, n_layers=12, d_ff=1536,
                 sigma=1.0, top_k=32, dropout=0.0, drop_path=0.0): # ImageNet ViT-S/16 defaults
        super().__init__()
