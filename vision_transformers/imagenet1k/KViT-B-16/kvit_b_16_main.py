...
def visualize_comprehensive_analysis(krause_results, save_path='vit_analysis.png'):
    ...
    ax1.plot(krause_results['train_accs'], 'o-', label='Krause-VT Train', linewidth=2)
    ax1.plot(krause_results['test_accs'], 's-', label='Krause-VT Test', linewidth=2)
    ax1.set_xlabel('Epoch')
    ax1.set_ylabel('Accuracy (%)')
    ax1.set_title('Training Progress (Krause-VT)')
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    
    ax2.plot(krause_results['train_losses'], 'o-', label='Krause-VT', linewidth=2)
    ax2.set_xlabel('Epoch')
    ax2.set_ylabel('Loss')
    ax2.set_title('Training Loss (Krause-VT)')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    
    plt.suptitle('Krause-VT (RBF/Top-K): Training Results',
...
def run_vit_evaluation(args):
    try:
        ...
        if is_main_process():
            ...
            print_rank0("\n" + "="*70)
            print_rank0("COMPUTING MODEL COMPLEXITY (Krause-VT)")
            print_rank0("="*70)
        ...
        print_rank0("\n" + "="*70)
        print_rank0("Training Krause-VT (with warmup)" if args.warmup_epochs > 0 else "Training Krause-VT")
        print_rank0("="*70)
        ...
        for epoch in range(args.epochs):
            ...
            if test_acc > best_krause_acc:
                ...
                if is_main_process():
                    torch.save(krause_model.module.state_dict(), 'best_krause_vt_imagenet.pth')
                    print_rank0(f"  → Saved best Krause-VT weights (Epoch {epoch+1}, Test Acc: {test_acc:.2f}%)")
        ...
        print_rank0("\n[Loading best Krause-VT weights]")
        state_dict = torch.load('best_krause_vt_imagenet.pth', map_location=device)
        ...
        best_krause_loss, best_krause_acc = evaluate(krause_model, test_loader, criterion, device,
                                                      num_classes=model_config['num_classes'],
                                                      label_smoothing=args.label_smoothing)
        print_rank0(f"Krause-VT Best Model (Epoch {best_krause_epoch}):")
        ...
        print_rank0(f"  Krause-VT: Best Test Accuracy = {best_krause_acc:.2f}% (Epoch {best_krause_epoch})")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train Krause-VT, Standard VT on ImageNet')
    ...
    parser.add_argument('--patch_size', type=int, default=16,
                       help='Patch size for VT (default: 16, VT-B/16 standard)')
    parser.add_argument('--d_model', type=int, default=768,
                       help='Embedding dimension (default: 768, VT-B-16)')
    parser.add_argument('--n_heads', type=int, default=12,
                       help='Number of attention heads (default: 12, VT-B-16)')
    parser.add_argument('--n_layers', type=int, default=12,
                       help='Number of transformer layers (default: 12, VT-B-16)')
    ...
    parser.add_argument('--save_path', type=str, default='vt_imagenet_krause.png',
