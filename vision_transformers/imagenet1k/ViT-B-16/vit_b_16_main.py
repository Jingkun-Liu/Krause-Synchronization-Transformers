...
def visualize_training(standard_results, save_path='vit_training.png'):
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))
    
    ax1.plot(standard_results['train_accs'], 'o-', label='Standard ViT Train', linewidth=2)
    ax1.plot(standard_results['test_accs'], 's-', label='Standard ViT Test', linewidth=2)
    ax1.set_xlabel('Epoch')
    ax1.set_ylabel('Accuracy (%)')
    ax1.set_title('Training Progress (Standard ViT)')
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    
    ax2.plot(standard_results['train_losses'], 'o-', label='Standard ViT', linewidth=2)
    ax2.set_xlabel('Epoch')
    ax2.set_ylabel('Loss')
    ax2.set_title('Training Loss (Standard ViT)')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    
    plt.suptitle('Standard ViT-B/16: Training Results',
...
def run_vit_evaluation(args):
    ...
    try:
        ...
        print_rank0("\n" + "="*70)
        print_rank0("STANDARD ViT-B/16 TRAINING")
        print_rank0("="*70 + "\n")
        ...
        print_rank0("\nInitializing Standard ViT-B/16 with optimizations...")
        
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        
        standard_model = vit_b_16(num_classes=model_config['num_classes'])
        
        if model_config['img_size'] != 224 or model_config['patch_size'] != 16:
            standard_model.conv_proj = nn.Conv2d(
                3, 768,
                kernel_size=model_config['patch_size'],
                stride=model_config['patch_size']
            )
            standard_model.hidden_dim = 768
            
            num_patches = (model_config['img_size'] // model_config['patch_size']) ** 2
            standard_model.encoder.pos_embedding = nn.Parameter(
                torch.randn(1, num_patches + 1, 768) * 0.02
            )
            standard_model.encoder.seq_length = num_patches + 1
            
            def custom_process_input(self, x):
                n, c, h, w = x.shape
                p = model_config['patch_size']
                expected_h = model_config['img_size']
                expected_w = model_config['img_size']
                torch._assert(h == expected_h, f"Wrong image height! Expected {expected_h} but got {h}!")
                torch._assert(w == expected_w, f"Wrong image width! Expected {expected_w} but got {w}!")
                n_h = h // p
                n_w = w // p
            ...
            import types
            standard_model._process_input = types.MethodType(custom_process_input, standard_model)
        ...
        if is_main_process():
            ...
            print_rank0("\n" + "="*70)
            print_rank0("COMPUTING MODEL COMPLEXITY (Standard ViT-B/16)")
            print_rank0("="*70)
        ...
        print_rank0("\n" + "="*70)
        print_rank0("Training Standard ViT (torchvision)" + (" with warmup" if args.warmup_epochs > 0 else ""))
        print_rank0("="*70)
        ...
        for epoch in range(args.epochs):
            ...
            if test_acc > best_std_acc:
                ...
                if is_main_process():
                    torch.save(standard_model.module.state_dict(), 'best_standard_vit_ImageNet_vitb16.pth')
                    print_rank0(f" Saved best Standard-ViT weights (Epoch {epoch+1}, Test Acc: {test_acc:.2f}%)")
        ...
        state_dict = torch.load('best_standard_vit_ImageNet_vitb16.pth', map_location=device)
        ...
        print_rank0(f"  Standard ViT: Best Test Accuracy = {best_std_acc:.2f}% (Epoch {best_std_epoch})")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Train Standard ViT-B/16 on ImageNet')
    ...
    parser.add_argument('--patch_size', type=int, default=16,
                       help='Patch size for ViT (default: 16, ViT-B/16 standard)')
    parser.add_argument('--d_model', type=int, default=768,
                       help='Embedding dimension (default: 768, ViT-B-16)')
    parser.add_argument('--n_heads', type=int, default=12,
                       help='Number of attention heads (default: 12, ViT-B-16)')
    parser.add_argument('--n_layers', type=int, default=12,
                       help='Number of transformer layers (default: 12, ViT-B-16)')
    parser.add_argument('--d_ff', type=int, default=3072,
                       help='Feed-forward network dimension (default: 3072, 768*4)')
    ...
    parser.add_argument('--save_path', type=str, default='vit_imagenet_vitb16.png',
                       help='Path to save visualization (default: vit_imagenet_vitb16.png)')
