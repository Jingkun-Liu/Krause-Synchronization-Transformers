...
class KrauseAttention(nn.Module):
    ...
    def forward_step(self, x, state=None, current_pos=0):
        ...
        k = self._split_heads(self.W_k(x)) # (B, H, 1, D_h)
...
class ImageTransformer(nn.Module):
    def __init__(self, vocab_size=256, seq_len=SEQ_LEN, d_model=512, nhead=8,
    ...
    def generate(self, device, num_samples, gen_batch_size, is_ddp=False):
        ...
        for batch_idx in tqdm(range(num_batches), desc=f"Batch Gen ({self.attn_type})"):
            ...
            generated = torch.zeros((current_batch_size, 1), dtype=torch.long, device=device)
...
def generate_only_and_plot():
    ...
    parser.add_argument('--d_model', type=int, default=256, help='Model dimension (D).')
    parser.add_argument('--nhead', type=int, default=8, help='Number of attention heads.')
    parser.add_argument('--num_layers', type=int, default=12, help='Number of transformer layers (L).')
    parser.add_argument('--top_k', type=int, default=96, help='Krause Attention Top-K parameter.')
    parser.add_argument('--window_size', type=int, default=128, help='Krause Attention Window Size parameter.')
    parser.add_argument('--generate_samples', type=int, default=50000, help='Number of images to generate per model.')
    parser.add_argument('--gen_batch_size', type=int, default=2000, help='Batch size for generation.')
    ...
    for attn_type in attn_types:
        ...
        samples = model_core.generate(device,
                                        num_samples=args.generate_samples,
                                        gen_batch_size=args.gen_batch_size)
        
        generated_images_np = samples.view(-1, IMAGE_H, IMAGE_W, IMAGE_C).cpu().numpy().astype(np.uint8)
        models_and_samples[attn_type] = torch.from_numpy(generated_images_np).view(-1, SEQ_LEN)


    if models_and_samples and 'krause' in models_and_samples:
        os.makedirs("generation_results_mnist", exist_ok=True)
        
        num_rows = 3
        num_cols = 8
        total_samples = num_rows * num_cols
        
        samples = models_and_samples['krause']
        plot_samples = samples[:total_samples].view(-1, IMAGE_H, IMAGE_W).numpy()
        ...
        for row in range(num_rows):
            for col in range(num_cols):
                ...
                if idx < len(plot_samples):
                    ax = axes[row, col]
                    ax.imshow(plot_samples[idx], cmap='gray')
                    ax.axis('off')
        
        plt.tight_layout()
        plot_path = "generation_results_mnist/generated_samples_mnist.png"
