...
class VanillaAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        V = V_full.view(B, T, self.num_heads, self.head_dim)

        mask_2d_causal = get_2d_causal_mask(T, device) # (1, 1, T, T)
        mask_2d_causal = mask_2d_causal.squeeze(0).squeeze(0) # (T, T)
        ...
        for h in range(self.num_heads):
            ...
            attn_scores_h = torch.matmul(Q_h, K_h.transpose(-2, -1)) / math.sqrt(self.head_dim)
            
            attn_scores_h = attn_scores_h + mask_2d_causal
            
            attn_weights_h = F.softmax(attn_scores_h, dim=-1)
            attn_weights_h = self.attn_dropout(attn_weights_h)
            
            context_h = torch.matmul(attn_weights_h, V_h)
            
            all_context.append(context_h)
...
class ImageTransformer(nn.Module):
    ...
    def __init__(self, vocab_size=256, seq_len=3072, d_model=256, nhead=8, num_layers=8, attn_type='vanilla', **kwargs):
        super().__init__()
...
def cifar10_transform_func(x):
    x = (x * 255).long()
    x = x.reshape(3, 32, 32)
    x = x.permute(1, 2, 0)
    return x.reshape(-1)

def get_cifar10_loaders(batch_size, rank, world_size):
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Lambda(cifar10_transform_func)
    ])
...
def run_benchmark(rank, world_size, args):
    ...
    if is_rank_0:
        print(f"{'='*50}")
        print(f"Starting DDP Benchmark on {world_size} GPUs (CIFAR-10 FIXED 2D MASK).")
        print(f"Krause: Top-K={args.top_k}, Window={args.window_size}")
        print(f"{'='*50}")

    train_loader, test_loader, train_sampler = get_cifar10_loaders(args.batch_size, rank, world_size)
    ...
    if is_rank_0:
        ...
        os.makedirs("benchmark_results_cifar10", exist_ok=True)
        
        plt.figure(figsize=(6, 5))
        for name, res in results.items():
            plt.plot(res['bpd'], label=name)
        plt.title('Bits Per Dimension (BPD) during Training (FIXED)')
        plt.xlabel('Epoch')
        plt.ylabel('BPD')
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.savefig("benchmark_results_cifar10/bpd_history_fixed.png")
        print("Results saved to benchmark_results_cifar10/")
...
def main():
    ...
    parser.add_argument('--top_k', type=int, default=192)
