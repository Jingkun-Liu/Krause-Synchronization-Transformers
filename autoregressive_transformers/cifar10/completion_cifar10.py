...
def get_test_loader(batch_size):
    transform = transforms.Compose([transforms.ToTensor(), transforms.Lambda(cifar10_transform_func)])
...
def load_model_from_file(path, args, device):
    print(f"Initialzing model: {args.attn_type}, D={args.d_model}, L={args.num_layers}, K={args.top_k}, W={args.window_size}")
...
def sequences_to_images(seqs):
    return seqs.reshape(seqs.shape[0], IMAGE_H, IMAGE_W, IMAGE_C).cpu().numpy().astype(np.uint8)
...
def plot_completions(original, completed, samples_per_img, filename):
    ...
    for i in range(num_imgs):
        ...
        for j in range(samples_per_img):
            axes[i, j+1].imshow(completed[i*samples_per_img + j])
            axes[i, j+1].set_title(f"Sample {j+1}")
            axes[i, j+1].axis('off')

        axes[i, -1].imshow(original[i])
...
def main():
    ...
    parser.add_argument('--model_path', type=str, default='saved_models_cifar10/cifar10_krause_D512_L16_K192_W256.pt', help='Path to the trained model checkpoint.')
    ...
    out_file = f"completion_{args.attn_type}_result.png"
