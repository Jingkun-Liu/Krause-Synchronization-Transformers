...
def train_and_benchmark(mode='krause', model_path="Qwen/Qwen2-7B", flanv2_path=None):
    KRAUSE_PARAMS = {
        'top_k': 16,
        'window_size': 32,
        'init_sigma': 5.5
    ...
    training_args = TrainingArguments(
        output_dir=f"./output_{mode}_qwen1_5_7b",
    ...
    cache_dir = "./flanv2_processed_qwen"
    ...
    if os.path.exists(train_cache_path):
        ...
        if GLOBAL_IS_MAIN_PROCESS:
            ...
            print_rank0(f">>> Found {len(all_json_files)} JSON files, loading...")
            ...
            eval_size = 5000
            train_size = 50000
            if len(train_ds) >= eval_size + train_size:
                eval_ds = train_ds.select(range(eval_size))
                train_ds = train_ds.select(range(eval_size, eval_size + train_size))
            else:
                eval_ds = train_ds.select(range(eval_size))
                train_ds = train_ds.select(range(eval_size, len(train_ds)))
                print_rank0(f">>> Warning: Data insufficient, actual training set {len(train_ds)} samples, evaluation set {len(eval_ds)} samples")
    ...
    if GLOBAL_IS_MAIN_PROCESS:
        model.save_pretrained(f"./final_model_{mode}_qwen1.5_7b")
        tokenizer.save_pretrained(f"./final_model_{mode}_qwen1.5_7b")
        print_rank0(f"Model saved to ./final_model_{mode}_qwen1.5_7b")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Train Krause Qwen model')
